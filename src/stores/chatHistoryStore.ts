...
export const useChatHistoryStore = defineStore({
    ...
    strategies: [
      {
        ...
        paths: ["chatList", "activeChatMenuId", "lastPageId"],
      },
    ],
  },
  ...
  getters: {
    ...
    activeChat: (state) =>
      state.chatList.find(
        (chat: Chat.Chat) => chat.chatId === state.activeChatMenuId
      ),
    consultantList: (state) =>
      state.chatList.filter(
        (chat: Chat.Chat) => chat.chatType === ChatType.Consultant
      ) as Chat.Chat[],
    creationList: (state) =>
      state.chatList.filter(
        (chat: Chat.Chat) => chat.chatType === ChatType.Creation
      ) as Chat.Chat[],
    baseList: (state) =>
      state.chatList.filter(
        (chat: Chat.Chat) => chat.chatType === ChatType.Base
      ) as Chat.Chat[],
  },
  actions: {
    // 添加聊天菜单
    ...
    deleteMenu(chatId: Chat.Id) {
      ...
      this.chatList = this.chatList.filter(
        (chat: Chat.Chat) => chat.chatId !== chatId
      ) as Chat.Chat[];
    ...
    updateMenuIsMenuEdit(chatId: Chat.Id, flag: boolean) {
      const targetChat = this.chatList.find(
        (chat: Chat.Chat) => chat.chatId === chatId
      ) as Chat.Chat;
      if (targetChat) {
    ...
    async readStream(reader) {
      const activeChat = this.chatList.find(
        (chat: Chat.Chat) => chat.chatId === this.activeChatMenuId
      );
