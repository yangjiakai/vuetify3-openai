...
export const useSpokenStore = defineStore({
    ...
    strategies: [
      {
        ...
        paths: ["spokenChatList", "activeChatMenuId", "lastPageId"],
      },
    ],
  },
  ...
  getters: {
    // 获取当前激活的聊天菜单
    activeChat(state) {
      return state.spokenChatList.find(
        (chat: Chat.SpokenChat) => chat.chatId === state.activeChatMenuId
      );
    },
  },
  actions: {
    ...
    deleteMenu(chatId: Chat.Id) {
      // 删除聊天菜单
      this.spokenChatList = this.spokenChatList.filter(
        (chat: Chat.SpokenChat) => chat.chatId !== chatId
      ) as Chat.SpokenChat[];
    ...
    updateMenuIsMenuEdit(chatId: Chat.Id, flag: boolean) {
      const targetChat = this.spokenChatList.find(
        (chat: Chat.SpokenChat) => chat.chatId === chatId
      ) as Chat.SpokenChat;
      if (targetChat) {
    ...
    addHistory(chatId: Chat.Id, message: Chat.SpokenMessage) {
      const currentChat = this.spokenChatList.find(
        (chat: Chat.SpokenChat) => chat.chatId === chatId
      ) as Chat.SpokenChat;
      if (currentChat) {
    ...
    startReading(messageId: Chat.Id) {
      const targetChat = this.spokenChatList.find(
        (chat: Chat.SpokenChat) => chat.chatId === this.activeChatMenuId
      );
      if (targetChat) {
        targetChat.messages.find(
          (message: Chat.SpokenMessage) => message.messageId === messageId
        ).isReading = true;
      }
    },
    ...
    async readStream(reader) {
      const activeChat = this.spokenChatList.find(
        (chat: Chat.SpokenChat) => chat.chatId === this.activeChatMenuId
      );
