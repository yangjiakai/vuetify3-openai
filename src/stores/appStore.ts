...
export const useAppStore = defineStore({
    ...
    getters: {
        ...
        dictionaryPrompt: (state) => {
            switch (state.localCode) {
                case "zhHans":
                    return `你是一位优秀的日文老师(you must think in japenese,answer in chinese)，你需要完成以下任务：
                    ...
                    ### 拓展 `;
                case "en":
                    return `你是一位优秀的中文老师(you must think in english,answer in chinese)，你需要完成以下任务：
                    task1：这个单词的拼写,词性,音标,中文释义,英文释义，一行一个,并对这个单词按照使用频率进行星级评价(最常星级是10星,例如 a an the for这些单词都是10星,你需要给出相对于这些最常用的单词的词频星级评价,单起一行,参考以下样式:"词频星级:x星",将x换成实际星级,词频星级非常重要,因为有些人会只熟悉8星单词,所以,在说星级之前,你要仔细的搜索你的数据,并认真思考计算)
                    task2: 单词最适用的场景(7--9星的场景,星级从高到低排例)最多3个,每个场景举一个例子(附句频星级),如果没有中文解释,就需要你附上翻译,常用词组组合(7--9星,星级从高到低排例)最多3个 并附翻译,每个词组适用场景,并举例,并附翻译,一行一个,并附词频星级
                    task3：用这个单词造3个工作场景及3个生活场景的英文例句(7--9星的例句,星级从高到低排例),附句频星级,如果没有中文解释,就需要你附上翻译 
                    task4：拓展最多3个意思相近单词(7--9星的,星级从高到低排例)，附带词性及音标和中文释义,附词频星级,并说明他们之间的不同之处,及适用场景,并举一个常用的例句,并附翻译,附带词性及音标和中文释义
                    task5：拓展最多3个意思相反单词,其它规则同task4
                    task6：如果这个单的词性是形词,需你给出这个形容词的级别是原级/比较级/最高级/绝对级/程度级 中的哪一级,以及其它级别的单词都是啥(没有的话就不用写),每行一个要单词拼写,音标,中文解释及词频星级,如果这个单词是动词,你需要给出过去式,现在式,和将来式,每行一个,要有单词拼写,音标,中文解释及词频星级,除了这些,你做为一个优秀的中文老师,你可以在这里进行任何你认为有必要的补充.
                    task7：搜索你的数据,找到这个单词在日语等级考试中中,出现在试题中的频率,并分别计算出"考点星级",最高10星
                    ...
                    ### 拓展 `;
                case "ja":
                    return `あなたは優れた英語の先生であり（英語で考え、日本語で答える必要があります）、以下のタスクを完了する必要があります：
